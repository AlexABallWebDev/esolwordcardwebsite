...
<html>
  ...
  <body>
                ...
                if ($result = @mysqli_query($cnxn, $sql))
                  ...
                  //while($row = mysqli_fetch_array($result)){
									...
									*/
									...
									//if no cards were found, a button to go to
									//create card page will be displayed.
									...
									foreach($result as $row)
									{
										...
										$cardFaceDivTag = "<div class='front'";
										
										//if this is the selected card from the table, change its color by giving it the selected-card id.
										$selectedCardID = $_GET['card_id'];
										if ($card_id == $selectedCardID)
										{
											$cardFaceDivTag = $cardFaceDivTag . " id='selected-card'";
										}
										$cardFaceDivTag = $cardFaceDivTag . ">";
										echo $cardFaceDivTag;
										...
										//echo "<div class='grade-info'>";
											//echo "<a href='delete-one-card.php?card_id=$card_id' class='label label-pill label-danger delete-single-card' data-toggle='popover'>Delete</a>";
										//echo "</div>";
										...
										echo "<div class='grade-info pull-right'>";
											echo "<a href='delete-one-card.php?card_id=$card_id' class='label label-pill label-danger delete-single-card' data-toggle='popover'>Delete</a>";
										echo "</div>";
					
										echo "</div>";
										$cardFaceDivTag = "<div class='back'";
									...
									//if no cards were found, display button to go to create card page.
									if (isset($emptyCardList))
									{
										print '<div class="row row-centered">';
										print '<div class="col-sm-12 col-md-12">';
										print '<p>You have no cards. You can create one by going to the create card page.</p>';
										print '<a href="create_card_form.php"><button class="btn btn-lg btn-main">Create a card!</button></a>';
										print '</div>';
										print '</div>';
										print '</div>';
									}
    ...
    <?php include 'footer.html';?>
		<script>
			//confirmation alert when deleting all cards as instructor
			...
			//confirmation alert when deleting single card
			$('.delete-single-card').on('click', function () {
